...
namespace qrcodegen {
	...
	// Returns a value in the range 0 to 3
	...
	{
		vector<QrSegment> segs = QrSegment::makeSegments(text); // 가장 효율적인 encoding을 자동으로 선택
		...
		int minVersion, int maxVersion, int mask, bool boostEcl)
	{
		...
		for (version = minVersion; ; version++) 
		{
			...
			if (dataUsedBits != -1 && dataUsedBits <= dataCapacityBits) // used bits < capacity bits 이면 사용 가능
			{
		...
		{
			if (boostEcl && dataUsedBits <= getNumDataCodewords(version, newEcl) * 8) // boostEcl이 뭐지
				ecl = newEcl;
		}
	...
	void QrCode::drawFinderPattern(int x, int y) 
	{
		for (int dy = -4; dy <= 4; dy++)
		{
			...
			{
				int dist = max(abs(dx), abs(dy));  // Chebyshev/infinity norm
				int xx = x + dx, yy = y + dy;
	...
	{
		return modules.at(y).at(x); // :vector<vector<bool> > 형식. 
	...
	bool QrCode::hasFinderLikePattern(const std::deque<int> &runHistory) {
		...
		return n > 0 && runHistory.at(2) == n && runHistory.at(4) == n && runHistory.at(5) == n
			&& runHistory.at(3) == n * 3 && max(runHistory.at(0), runHistory.at(6)) >= n * 4;
	}
